...
let est28 = document.querySelector("#estudiantes28");
/* lista.innerHTML = `
...
datos.forEach((estudiante) => {
  renderizaSkeleton(lista);
  if (estudiante.pais === "Argentina") {
    renderizaSkeleton(estChile);
  }
  if (estudiante.edad === 28) {
    renderizaSkeleton(est28);
  }
});
...
async function renderizarDatos() {
  try {
    ...
    estudiantes.forEach((estudiante) => {
      renderizaCard(lista, estudiante);
      if (estudiante.pais === "Argentina") {
        renderizaCard(estChile, estudiante);
      }
      if (estudiante.edad === 28) {
        renderizaCard(est28, estudiante);
      }
    });
  } catch (error) {
    console.log(error);
...
function renderizaCard(elemento, estudiante) {
  elemento.innerHTML += `<a href="#" class="flex flex-col max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
<h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">${estudiante.nombre}</h5>
<p class="font-normal text-gray-700 dark:text-gray-400">${estudiante.edad} - ${estudiante.pais}</p>
</a>`;
}

function renderizaSkeleton(elemento) {
  elemento.innerHTML += `<div class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
    <div role="status" class="max-w-sm animate-pulse">
        <div class="h-2.5 bg-gray-200 rounded-full dark:bg-gray-700 w-48 mb-4"></div>
        <div class="h-2 bg-gray-200 rounded-full dark:bg-gray-700 max-w-[360px] mb-2.5"></div>
        <div class="h-2 bg-gray-200 rounded-full dark:bg-gray-700 mb-2.5"></div>
